...
class StockOutRepository implements IStockOutRepository {
  ...
  public async getStockOuts(payload: GetStockOutsPayload): Promise<{ results: StockOutEntity[]; count: number }> {
  ...
  public async createStockOut(data: CreateStockOutPayload): Promise<StockOutEntity> {
    // Generate a new ID
  ...
  public async updateStockOutStatus(id: string, status: StockOutEntity['status']): Promise<StockOutEntity> {
