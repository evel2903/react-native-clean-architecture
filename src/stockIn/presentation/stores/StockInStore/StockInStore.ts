...
import { IStockInRepository, IStockInRepositoryToken } from "src/stockIn/domain/specifications/IStockInRepository";
...
export class StockInStore implements StockInStoreState {
  ...
  constructor(
    @inject(GetStockInsUseCase) private readonly getStockInsUseCase: GetStockInsUseCase,
    @inject(CreateStockInUseCase) private readonly createStockInUseCase: CreateStockInUseCase,
    @inject(IStockInRepositoryToken) private readonly stockInRepository: IStockInRepository
  ) {
  ...
  mergePagination = (payload: Partial<StockInStoreState["pagination"]>): void => {
  ...
  async getStockIns() {
    ...
    try {
      ...
      this.setError(error instanceof Error ? error.message : "Failed to fetch stock ins");
    } finally {
  ...
  async createStockIn() {
    ...
    try {
      // Validate form data
      if (!this.formData.productId || !this.formData.productName || this.formData.quantity <= 0) {
        this.setError("Please fill all required fields");
        return null;
      }
      
      const newStockIn = await this.createStockInUseCase.execute(this.formData);
      ...
      this.setError(error instanceof Error ? error.message : "Failed to create stock in record");
      return null;
    } finally {
      this.setIsLoading(false);
    }
  ...
  async getStockInDetails(id: string) {
    ...
    try {
      ...
      this.setError(error instanceof Error ? error.message : "Failed to fetch stock in details");
      return null;
    } finally {
      this.setIsLoading(false);
    }
  ...
  async updateStatus(id: string, status: StockInEntity['status']) {
    ...
    try {
      const updatedStockIn = await this.stockInRepository.updateStockInStatus(id, status);
