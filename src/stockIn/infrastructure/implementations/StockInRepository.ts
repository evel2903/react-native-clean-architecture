...
class StockInRepository implements IStockInRepository {
  ...
  public async getStockIns(payload: GetStockInsPayload): Promise<{ results: StockInEntity[]; count: number }> {
  ...
  public async createStockIn(data: CreateStockInPayload): Promise<StockInEntity> {
    // Generate a new ID
  ...
  public async updateStockInStatus(id: string, status: StockInEntity['status']): Promise<StockInEntity> {
